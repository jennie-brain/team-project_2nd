...
> # 🚫 기각 (2026-08-19)
>
> **이 문서는 기각되었습니다.** 팀이 *"서비스가 과거 소비 데이터를 가져와야 하므로 **마이데이터를 사용한다**"* 로 결정하면서 이 작업안의 전제가 무효화되었습니다.
>
> | | 이 문서의 전제 (08-18) | 08-19 팀 결정 |
> | --- | --- | --- |
> | 데이터 획득 | MVP는 마이데이터 미연동, 사용자 직접 입력 (쟁점 1) | **마이데이터 연동** — 카드 이용내역은 오픈뱅킹이 아닌 마이데이터 API로만 제공되므로 과거 소비를 가져오려면 필수 |
> | TAM 앵커 | 국내 신용카드 실사용자 전체 | **마이데이터 이용자** |
>
> 무효화된 핵심 논거: **§0-3**("마이데이터 수치를 TAM 근거로 쓰면 안 되는 이유") 전체, **§2-1** TAM 산식(신용카드 실사용자 기준), **§0-1** 교정 구조의 TAM 최상단 정의.
>
> 이력 보존을 위해 아래 본문은 수정·삭제하지 않고 그대로 둡니다 — 근거는 `decision-log/decision-log.md` 2026-08-19 표 참고.
>
> **팀 확인 사항**: TAM 앵커와 무관한 부분(**§1** 7단계 리서치 절차, **§2-2** 대리지표 중복 제거 규칙과 보수/기본/공격 3시나리오 민감도, **§4** Segment Map 축 제안)은 방법론상 여전히 유효합니다. 04 담당이 필요하다고 판단하면 새 문서로 이관할지 결정해 주세요 — 이 문서를 되살리는 방식은 피합니다.
...
```mermaid
flowchart TD
    TAM["TAM — 국내 개인 신용카드 사용자 전체
    (결제수단 선택권을 가진 모든 사람)"]
    SAM1["SAM Phase 1 — 12개월 내 소비구조가 바뀌는 카드 사용자
...
### 0-3. 마이데이터 수치를 TAM 근거로 쓰면 안 되는 이유

쟁점 1의 판단은 "**MVP는 마이데이터 연동 없이 사용자 직접 입력**"입니다. 그렇다면 마이데이터 가입자 수는 우리 제품의 수요 모집단이 아니라 **경쟁 서비스의 데이터 인프라 지표**입니다. TAM 모수는 신용카드 이용자 쪽이어야 논리가 연결됩니다.

- 마이데이터 수치는 삭제하지 말고 **Phase 2(연동 확장) 실현가능성 근거**로 위치를 옮길 것
- 리서치 8장도 같은 취지로 "TAM 재확인 필요"를 이미 지적함 (1.77억 건 vs 1.65억 명 집계 기준 상이)
...
**정의문 초안**: *국내에서 개인 명의 신용카드를 보유하고 결제수단을 스스로 선택할 수 있는 사용자 전체.*

```
TAM_user = 국내 개인 신용카드 실사용자 수 (명, 중복 제거)
TAM_₩    = 연간 개인 신용카드 국내 결제액 × 평균 미획득 혜택률
```

| 변수 | 현재 값 | 구분 | 확보 방법 |
| --- | --- | --- | --- |
| 개인 신용카드 실사용자 수 | 🟠 미확보 | — | 발급매수 1억 3,466만 매 ÷ 1인당 평균 보유 장수, 또는 경제활동인구 × 신용카드 보유율. **둘 다 구해 교차검증** |
| 1인당 평균 카드 보유 장수 | 🟠 미확보 | — | 여신금융협회 / 한국은행 지급수단 이용행태 조사 |
| 연간 개인카드 결제액 | 개인카드 승인금액 273.7조원 (2026 2Q) | 🔵 Fact | 4개 분기 실적으로 연간 집계 (⚠️ 단순 ×4 금지 — 계절성) |
| 평균 미획득 혜택률 | 🟡 가정 | 🟡 | 전월실적 미달·통합할인한도 초과로 놓치는 비율. 카드사 약관 + 자체 시뮬레이션으로 하한선 추정 |

> **금액 TAM을 "매출"이 아니라 "가치풀"로 정의하는 이유**: 우리 서비스가 가져가는 건 결제액이 아니라 *그 결제에서 새는 혜택*입니다. "연간 개인카드 결제액 ○○조원 중, 조건 미달로 놓치는 혜택이 △△조원 — 이것이 우리가 되찾아 주는 시장"이 훨씬 방어 가능한 서술입니다. 단, 미획득 혜택률은 🟡이므로 **보수적 하한값**만 쓸 것.

### 2-2. SAM (Bottom-up, 대리지표 합산 후 중복 제거)

**정의문 초안**: *향후 12개월 내 소비 구조가 유의미하게 바뀔 것으로 예상되는 국내 개인 신용카드 사용자.*

```
SAM_user = [ Σ(연간 소비변화 이벤트 발생 인원) − 중복 ] × 신용카드 보유율
...
| --- | --- | --- | --- |
| **TAM** | 국내 개인 신용카드 사용자 / 연간 개인카드 결제액 | 여신금융협회 월간 통계(crefia.or.kr), 한국은행 지급결제통계 | 통계청 KOSIS |
| **SAM** | 12개월 내 소비구조 변화 이벤트 발생 카드 사용자 | 통계청 KOSIS(혼인·이혼 / 국내인구이동 / 출생), 행안부 주민등록 | 한국은행 가계금융복지조사 |
