...
- **우리 체인**: **미래 의도 포착**(사용자가 미래 소비 변화를 직접 입력) → **시나리오 모델링**(제약조건+시나리오 계산) → **제약 최적화**(해지·유지·신규를 함께 계산해 Net Benefit이 가장 큰 하나의 조합으로 압축) → **결제수단 배분** — 4단계

경쟁자 체인에는 뒤 두 단계(제약 최적화, 결제수단 배분)가 아예 없다 — "과거 소비 조달 → 단일 카드 매칭"만 있고 끝난다(🔵).
...
| 🏗️ 카드 해지를 서비스 범위에서 제외하는 이유 | 카드 해지 절차 자체의 마찰(상담사 만류·복잡한 절차)이 실제 민원으로 확인된다(🔵). 해지 상담·해지 신청 대행 등 실행 지원은 어떤 형태로도 제공하지 않는다 — 카드 신청·해지는 전적으로 사용자가 카드사를 통해 직접 진행한다. "이 카드를 해지하세요"라고 계산 결과를 제안하는 것과, 실제 해지 절차를 대신 처리하거나 안내하는 것은 다른 일이며, 후자는 카드사의 책임 영역이다 |
...
| 서비스 | 해지 실행을 서비스 범위로 착각하면 사용자 기대와 서비스 능력 사이 괴리를 만듦 | 서비스 범위를 "계산 근거 제공까지"로 명확히 제한 |
