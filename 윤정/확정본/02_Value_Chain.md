...
flowchart TB
    ...
    subgraph PA["⚙️ 주요활동"]
        direction LR
        IL["투입·조달 물류<br/>(마이데이터 연동 과거소비+보유카드)"] --> OP["운영·생산<br/>(계산엔진)"] --> OL["산출·배송 물류<br/>(Payment Plan 결과 전달)"] --> MS["마케팅·영업<br/>(혼인 Beachhead 채널)"] --> SV["서비스<br/>(오류신고·해지 유의사항 안내)"]
    end
    SA --> PA
...
표준 가치사슬 5개 활동 버킷(투입물류·운영·산출물류·마케팅·서비스) 안에 차별점이 흩어져 있어 한눈에 보이지 않는다. 실제로 경쟁을 가르는 것은 다음 3단계다.

- **경쟁자(뱅크샐러드·토스·카카오페이) 체인**: 과거 소비 조달(마이데이터) → 단일 카드 매칭(Rule 대입) → 결과 표시(발급 연계) — 2단계
- **우리 체인**: **미래 의도 포착**(사용자가 미래 소비 변화를 직접 입력) → **시나리오 모델링**(제약조건+시나리오 계산) → **제약 최적화**(해지·유지·신규를 함께 계산해 하나의 결론으로 압축) — 3단계

경쟁자 체인에는 앞뒤 두 단계(미래 의도 포착, 제약 최적화)가 아예 없다 — "과거 소비 조달 → 단일 카드 매칭"만 있고 끝난다(🔵).
...
| 🏗️ 흐름 설계 | 마이데이터 연동(과거 소비·보유카드 확인) → 미래 소비 변화 입력 → 제약조건 입력 → 시나리오 계산 → 해지·유지·신규추가를 함께 계산해 하나의 최적 조합으로 압축 → 결제수단 배분 → 근거 공개 |
...
| 구분 | 내용 |
| --- | --- |
| 🏗️ 결과 형식 | 계산으로 압축된 **하나의 최종 추천 조합**(예: "A·B카드 해지, C카드 유지, D카드 신규 추가"), 카드별 역할·배분금액, 지금 그대로 두면 놓치는 금액과 계산 근거, 적용되지 않은 조건을 함께 제시한다 |
...
### 5. 서비스 — 계산 오류 신고·해지 유의사항 안내

| 구분 | 내용 |
| --- | --- |
| 🏗️ 필요한 사후지원 | 계산 오류 신고·정정 절차, 해지 시 유의사항(포인트 소멸·자동납부 승계 여부) 안내 |
| 🏗️ 스코프 한정 이유 | 카드 해지 절차 자체의 마찰(상담사 만류·복잡한 절차)이 실제 민원으로 확인되어(🔵), 결제수단 배분 기능은 "판단 근거 + 유의사항 안내"로 한정한다 |
...
| 🚀 성장 vs 리스크 기준 | 규제 리스크(재무자문 분류 여부)가 해소되기 전까지는 리스크를 우선한다 |
...
| 서비스 | 해지 실행 마찰이 사용자 기대와 서비스 능력 사이 괴리를 만듦 | 결제수단 배분 스코프를 "판단 근거 + 안내"로 명확히 제한 |
