...
flowchart TD
    ...
    B --> C["미래지출 입력<br/>카테고리·금액·시점<br/>(적게·예상대로·많이 3개 시나리오로 계산)"]
...
<span style="color:red;">이 흐름은 지출 시나리오(적게·예상대로·많이) 각각에 대해 독립적으로 실행된다. 화면은 항상 예상 지출액 기준 결론 하나로 열리고, 적게·많이 시나리오는 탭으로 전환해 확인한다.</span>
