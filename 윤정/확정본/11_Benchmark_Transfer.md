...
뱅크샐러드도 이미 현재 소비 패턴에 맞는 최적 카드 조합을 추천하므로, 경쟁 구도를 "단일 카드 추천 vs 카드 조합 추천"으로 정의하는 것은 정확하지 않다. 실제 경쟁 포인트는 다음과 같다.

> **과거 행동을 이용한 사후적 카드 구성 최적화 → 미래 행동을 입력받아 의사결정 전에 수행하는 선제적 카드 구성 최적화**

차별점은 카드의 개수가 아니라 **시간축과 의사결정 방식**이다. 이 서비스의 핵심은 카드 추천 알고리즘 자체가 아니라 **과거 행동 + 미래 의도 → 시나리오 → Net Benefit → 결정**으로 이어지는 의사결정 체계다.
