...
| — | 통신 요금제 진단(스마트초이스) | 사용 패턴 기반 상품 재매칭 — 핀트와 동일 메커니즘 | 🔶 제외 |
| — | TMAP TMS(배차·경로 최적화) | 다대다 제약 최적화 — 카드 조합과 구조 불일치 | 🔶 제외 |

동일·인접 금융 서비스뿐 아니라 교차 산업(통신, 물류)도 검토했다. 통신 요금제 진단은 계산 구조가 카드 조합 문제와 같지만 이미 핀트 사례로 채택한 메커니즘과 겹쳐 별도로 반영하지 않았고, TMAP TMS는 이동거리·순서가 핵심인 구조라 실적구간을 채우는 카드 조합 문제와 맞지 않아 배차·경로라는 은유를 가져오지 않았다.
...
마지막 단계(M)는 신청서 작성·제출을 대신하지 않는다. 뱅크샐러드와 동일하게 카드사의 공식 신청 페이지로 연결하는 링크만 제공하며, 해지는 이 단계 대상이 아니다 — 해지 상담·대행은 여전히 서비스 범위 밖이다.

### 지출 변동 커버리지

입력한 예상 지출액 하나만으로는 실제 지출이 그보다 적거나 많을 때의 결과를 알 수 없다. 이를 보완하기 위해 예상 지출액 기준 시나리오 외에 적게 쓴 경우·많이 쓴 경우 두 시나리오를 함께 계산해 탭으로 제공한다. 화면은 항상 예상 지출액 기준 결론 하나로 먼저 열리며, 나머지 두 시나리오는 사용자가 원할 때만 탭으로 열어보는 보조 확인 화면이다 — 3개 안을 나열해 고르게 하지 않는다는 원칙은 그대로 유지된다.
