...
| 신용카드 발급매수 | 1억 3,466만 매(2025) | 카드 장수, 1인 다중보유 포함 | ✗ 사람 수 아님 — 08.19 재검토로 TAM에서 제외 |
| 개인카드 국내 이용액 누계 | 약 275.97조원 | 누계 거래액 | ✗ 사람 수 아님 — 금액 TAM(가치풀) 계산용으로만 사용 |

**직접 판단**: 마이데이터 가입 건수만이 "지출을 능동적으로 확인하는 사람"이라는 서비스 자격 기준과 직접 연결된다. 신용카드 발급매수는 카드 보유 여부만 나타내고 마이데이터 이용 여부와는 무관하므로 08.18~08.19 재검토를 거쳐 TAM 후보에서 제외했다(아래 2절 참고).
...
**단위·기간·지표 고정**:
- 단위: **사용자 수(명)** — 금액(₩)은 가치풀(value pool) 개념으로 별도 산정(6절 참고), BM이 미정(일회성 vs 지속형)이라 매출 기준 TAM은 가정 위에 가정을 쌓기 때문
- 지역: 국내
- 기간: 2026년 기준
...
| 1인당 연간 개인카드 결제액 | 273.7조원(2026 2Q 승인금액) ÷ 신용카드 실사용자 수 | 🔵(분자)/🟠(분모) | 분모(신용카드 실사용자 수)는 발급매수÷1인당 평균 보유장수 또는 경제활동인구×보유율로 교차검증 필요 |
| 평균 미획득 혜택률 | 🟡 가정 | 🟡 | 전월실적 미달·통합할인한도 초과로 놓치는 비율 — 보수적 하한값만 사용 |

**이 정의가 최종인 이유 (재논의 이력, 재논의 금지)**:

| 단계 | TAM 정의 | 폐기/유지 이유 |
| --- | --- | --- |
| 최초 | 마이데이터 가입자 | 포함관계 오류(SOM>SAM)로 폐기 — 앵커 자체는 유지 |
| 08.18 2차 교정 | 국내 신용카드 실사용자 전체 | 쟁점 1(당시 "MVP는 마이데이터 미연동")과 정합을 맞추려 교정, 그러나 신용카드 실사용자는 채택 가능성 낮은 인구·디지털 비친화 인구까지 과다포함해 반박 발생 |
| 08.19 3차 교정(현재) | **마이데이터 이용자** | 두 반박(과다포함·필터링 기능 부재)을 근거로 되돌림. 이 시점에 남아있던 쟁점 1과의 모순은, 같은 날 쟁점 1 자체를 "MVP부터 마이데이터 연동 포함"으로 재정정하면서 완전히 해소됨(`decision-log/decision-log.md` 2026-08-19) — **TAM=마이데이터 이용자는 이제 문자 그대로의 이용 자격 기준이며, 더 이상 워크어라운드가 아님** |

> 이 문서 이후로 TAM 앵커는 재논의하지 않는다. 남은 작업은 고유인원 환산 방식(🟠)을 확정하는 정량 검증뿐이다.
...
> TAM이 이미 마이데이터 이용자로 좁혀졌으므로, 대리지표의 모수도 "신용카드 보유율"이 아니라 **"마이데이터 이용률"**로 필터링한다(08.19 갱신, TAM 재확정에 따른 산식 연동).
...
| SOM | SAM(Phase 1) 중 Beachhead(혼인) 채택자 | 세 변수(비중·도달률·선택률) 모두 미확정, 카드고릴라 MAU 140만·뱅크샐러드 130만을 상한 벤치마크로만 참고 | 🟡 가정(Concierge Test로 채울 예정) |
...
| SOM | 혼인 Beachhead 채택자 | 🟡 3변수 모두 Concierge Test 대기 |
