...
**약해지는 조건**:
- 방어선은 지금 카드를 그대로 쓰면 이번 지출에서 얼마를 놓치고 있는지를 손실로 보여주는 것이다 — `윤정/0819/메커니즘_벤치마킹_분석_리포트.md` 1절(뱅크샐러드 벤치마킹)이 "지금 이 카드로는 혜택 0원, 다른 카드로 바꾸면 8,000원 더 받는다"처럼 놓치는 돈을 구체적으로 보여주는 방식으로 이미 설계해뒀다. 이 손실 가시화가 "아무것도 안 하기"라는 대체재를 흔드는 동기 요인이다
