...
> **Master Deck 페이지 원고.** 원본 작업본은 `윤정/0819_01_Porters Five Forces 분석.md`(윤정, 방법론 01) — 이 문서는 그 분석에 08.19 검증 결과 5건을 반영한 발표 원고입니다. 분석 자체는 윤정 산출물이고, 여기서 바꾼 것은 아래 "수정 이력"에 전부 명시했습니다.
>
...
| # | 수정 내용 | 근거 |
| --- | --- | --- |
| 1 | **카드고릴라를 마이데이터 활용 사업자에서 제외** — 5절 "4곳 모두 마이데이터 활용" → "3곳 활용, 카드고릴라는 비마이데이터 미디어·비교 모델" | 인용 출처 `국내 신용카드 시장 리서치.md` 3절 표의 카드고릴라 행에 마이데이터 언급 없음(트래픽·고릴라차트 지표만). 웹 검증에서도 본인신용정보관리업 인가 근거 미확인 🔵 |
| 2 | **뱅크샐러드 "실행설계 ✗" → "◐ 부분"으로 재판정** + 재판정 근거 명시 | 리서치 문서가 2회 요청한 재검증(29·224행) 이행. 2026.05 출시 보도 확인 🔵 |
| 3 | **마이데이터 본허가 "69개사" 🔵 → 🟠 강등**, 상충 수치 병기 | 자체 리서치 201행이 "2026.08 현재 사업자 수 🟠 확인 안 됨"으로 명시. 외부 자료는 35곳(2024.03) 제시 — 집계 기준 상이 가능 |
| 4 | 깨진 문자 2곳 치환 (`경�쟁`·`경�자` → `경쟁`·`경쟁자`) | 제출물 표기 정합 |
| 5 | 인용 출처의 무효 서술에 각주 추가 | `국내 신용카드 시장 리서치.md` 28·222행 *"MVP는 마이데이터 미연동 판단이 규제 현실과 일치"* 는 08.19 결정으로 무효 |

> ⚠️ **수정 2는 팀 승인 대기**입니다 — 경쟁사 비교표는 브리프 2절·피치덱과 공유되는 표라 이 원고에서 단독 확정할 수 없습니다. 브리프 원저자(윤정) 확인 후 확정하십시오.
...
> ※ 각주: 인용 출처 `국내 신용카드 시장 리서치.md` 28·222행의 *"MVP는 마이데이터 미연동이 규제 현실과 일치"* 서술은 **08.19 결정(MVP에 마이데이터 연동 포함)으로 무효**입니다. 해당 문서의 규제 사실관계(오픈뱅킹 불가)만 유효합니다.
...
**약해지는 조건**
- 단종카드 자동대체 이후 "혜택 불일치" 민원 다수(🔵 민원 2022→2025 +88.4%) — **카드사의 일방적 자동대체에 대한 불만이 이미 존재**한다는 것은 "근거를 보여주는 재배치"(차별점 C)가 이 대체재의 약점을 파고들 여지가 있다는 신호

**판단**: **높음**이나 고정되지 않음 — 차별점 C가 대체재 대비 우위를 만드는지가 검증 가설로 남음.
...
**약해지는 조건(경쟁 완화)**
- 경쟁 6곳 중 3조건 동시 충족자 없음 — 아래 재판정 반영
- 유사 기능(미래 소비항목 반영)을 제공했던 더쎈카드는 2023년 출시 후 2026년 서비스 종료(원인 🟠 미확인) — 이 세그먼트에 확신 있는 성공 사례가 아직 없음

### 뱅크샐러드 3조건 재판정 (🟠 팀 승인 대기)

리서치 문서가 2회 요청한 재검증을 이행한 결과입니다.

| 조건 | 기존 | 재판정 | 근거 |
| --- | :---: | :---: | --- |
| 실행 설계 | ✗ | **◐** | 발급 연계까지 지원하므로 "추천에서 끝"은 아님 🔵. 그러나 **보유카드 재배치·결제수단 배분은 없음** — 실행의 종착점이 신규 발급이지 조합 재설계가 아님 |
| 미래 소비 반영 | ✗ | **✗ 유지** | 마이데이터로 연결된 **최근 1년 소비 내역**을 기준으로 계산 🔵 — 미래 입력을 받는 단계가 없음 |
| 근거 공개 | ◐ | **◐ 유지** | "얼마를 더 받을 수 있는지 **원 단위까지**" 결과 금액은 공개 🔵. 그러나 공개되는 것은 **결과**이고 실적 구간·한도가 어떻게 적용됐는지의 **산출 근거**는 아님 |

**결론: 3조건 동시 충족은 여전히 아님.** 다만 논거를 *"뱅크샐러드는 실행을 안 한다"* 에서 *"뱅크샐러드의 실행은 신규 발급으로 끝나고, 계산 기준은 과거 1년이며, 후보 카드 **단 한 장**과 비교한다"* 로 바꿔야 정확합니다 — 후자가 훨씬 방어력이 강하고, **조합 배분(F-05)이 독립 차별축임을 외부 보도로 입증**합니다.
...
**연결 문서**: `윤정/0819_01_Porters Five Forces 분석.md`(원본 작업본) · `윤정/0819_03_KSF 분석.md`(후속) · `./README.md`(통합 지도)
