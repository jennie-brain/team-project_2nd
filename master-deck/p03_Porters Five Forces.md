...
- 마이데이터 본허가 사업자는 **69개사**(허가심의 중 30·예비허가 3·허가신청 27, 🔵 신용정보협회·마이데이터플랫폼) — 전 금융업권을 합쳐 이 규모이므로 채널이 좁다
- API 과금 부담이 실재한다 — "사용자가 늘수록 적자가 커진다"는 구조적 한계가 업계에서 공개적으로 지적된다(🔵 전자신문 2025.05.15). 그중 7개사가 과금 부담으로 허가를 반납했다는 보도가 있으나 반납 건수 자체는 🟠 미확인
...
- 경쟁 6곳 중 3조건(실행설계·미래소비반영·근거공개)을 동시 충족하는 곳이 없다 🔵 — 가장 앞선 뱅크샐러드도 아래 평가에서 충족하지 못한다
  - **6곳 = 뱅크샐러드·카드고릴라·더쎈카드·토스·Credit Karma·NerdWallet.** 카카오페이는 이 모집단에 포함되지 않는다 — `가람/0820/경쟁사 분석 결과.md`의 **별도 5곳 세트**(뱅크샐러드·카드고릴라·토스·카카오페이·더쎈카드)와 모집단이 다르므로 섞어 쓰지 않는다
  - **개별 3조건 판정은 뱅크샐러드 한 곳만 채운다** — 나머지 5곳은 집계 결론만 있고 개별 판정 근거가 없어, 추정으로 메우지 않는다
