...
flowchart LR
    ...
    Ext -.->|"F-05 안내로 실행 마찰 완화 시<br/>신뢰 회복 여지"| Core
...
> 신은서는 신뢰도·관여도만으로는 정하늘과 유사한 위치에 놓이지만, **"계산 결과를 실행까지 완주하는가"**라는 제3의 축에서만 갈립니다. 2축 지도로는 표현되지 않는 이 축이 F-05(결제수단 배분 설계) 스코프를 "안내"로 한정한 실제 근거입니다.
...
| 신은서 (Extreme) | 이서연 (Core)로 회복 | F-05를 "해지 대행"이 아니라 "판단 근거 + 유의사항 안내"로 좁혀 실행 마찰을 낮춤 | Guardrail G-04 |
...
> 이 보고서는 `가람/0819_페르소나.md`(스펙트럼 도출 → Convergent Filtering → Contextual Rewriting → AI Peer Review → Spectrum Map, 5단계 파이프라인 산출물)와 **별개의 문서**입니다. 그쪽은 4명의 대표 페르소나로 수렴시킨 최종 통합본이고, 이 보고서는 Market Segment Map(`윤정/0818_쟁점정리.md` 쟁점 3, Q1~Q4·SAM Phase1/2)에 근거해 **핵심 5·확장 3·극단 2·비활성 2, 총 12명**을 각자 이름·직무·나이·기술 수준·문제(Pain)·목표(Goal)·사용 상황(Context)·감정·대체 솔루션(Competitor)·기대 개선점(Expected Gain)까지 상세화한 독립 분석입니다.
>
...
**연결 문서**: `윤정/0818_쟁점정리.md`(쟁점 3, Segment Map) · `효진/컨설팅 전 1페이지 브리프.md`(문제③, 가장 큰 가정) · `가람/0819_페르소나.md`(별도 5단계 파이프라인·4명 대표 최종본) · `방법론.md` 5장·8장
...
| 신은서 | ⑥ 실행(해지 시도)에서 실패 | 계산 결과를 실행까지 완주하지 못함 — 상담사의 만류·전환유도 화법에 부딪힘 | 해지 실행 전 "예상되는 만류 포인트와 대응 방법"을 미리 안내하는 기능 검토(단, F-05 스코프상 "대행"이 아니라 "안내" 범위 내에서) |
...
**연결 문서**: `가람/0819_고객 여정지도 작성 방법론.md`(절차서) · `가람/0819_페르소나 스펙트럼 분석 보고서.md`(12명 상세) · `가람/0819_페르소나.md`(4명 대표 통합본) · `가람/0819_AOS 분석 결과.md`(0절에 12명 대표 Pain Point 3개씩 + Goal 추론 통합, 06 시장기회 분석 자료) · `윤정/0818_쟁점정리.md`(F-01~F-06, Guardrail G-04)
