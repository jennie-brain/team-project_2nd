...
> `방법론.md` 06(기회점수 OS 기반 시장기회 분석) 산출물(요구 5~10개 중요도·만족도·OS와 우선순위)을 만들기 위한 절차서입니다. `가람/0819/페르소나 스펙트럼 분석 보고서.md`·`가람/0819/페르소나.md`·`가람/0819/고객 여정지도.md`(특히 14절)를 입력 재료로 삼습니다.
>
...
> ⚠️ 정정: 이 문서의 이전 버전은 원문 미제공으로 ODI 표준식(`Importance + max(Importance−Satisfaction, 0)`)을 임시로 썼으나, 팀이 실제 사용할 공식을 확인해 위 식으로 교체합니다.

Satisfaction이 5점 만점 기준 낮을수록(즉 `1 − Satisfaction/5`가 1에 가까울수록) Importance가 그대로 살아남는 구조 — 중요한데 충족되지 않은 문제일수록 점수가 커집니다. "Adjusted"는 이 프로젝트 특성(가상 페르소나 기반, 실측 데이터 부족)에 맞춘 가중치 보정을 3절 4단계에서 반영한다는 의미로 씁니다.
...
가장 단순하고 직관적인 방식. Likert 5점 척도(1~5)를 그대로 쓰고, 중간값 **3.0**을 중심선으로 삼습니다.
...
📘 **활용법**: 문제의식 및 솔루션의 유형에 따라 기준을 조정해도 괜찮습니다(예: "Importance가 4 이상인 Pain만 상단으로 올리자"). 목표는 구조적 사고 훈련 — 문제 해결을 구조화하는 데에 초점을 둡니다.

**이 프로젝트에서의 적용**: 실측 데이터가 아직 없는 지금 단계는 방법 A를 채택합니다. Satisfaction·Importance 기준선 3.0, AOS 기준선 2.0~2.5를 4절(다음 액션) 채점의 임시 기준점으로 사용합니다.

#### 💡 B. 기존 사업에서의 데이터 분포에 따라 수립

이미 사업 운영 중이고 여러 차례 도출된 점수 분포가 존재하는 경우, 평균값과 표준편차를 이용해 사분면을 데이터 기반으로 자동 구분할 수 있습니다.
...
📘 **해석 포인트**: "우리 사업의 실제 평가 분포"에 맞게 사분면이 형성되므로 페르소나나 산업별 편향을 줄일 수 있습니다.

**이 프로젝트에서의 적용**: 아직 표본이 5~10개뿐이라 평균·표준편차가 안정적이지 않습니다. 07 JTBD 인터뷰·실측 설문으로 채점 표본이 쌓이면(예: 20건 이상) 방법 A에서 방법 B로 전환합니다.
...
| --- | --- | --- | --- |
| 페르소나 단계 | 각 페르소나의 주요 Pain·Goal | "이 사람에게 가장 중요한 고통은 무엇인가?" | `가람/0819/페르소나 스펙트럼 분석 보고서.md` 12명의 Pain(문제)·Goal(목표) 필드 |
| 고객 여정 지도 | 여정 단계별 Pain Point / 개선기회 | "고객 여정 중 어디서 좌절이 가장 큰가?" | `가람/0819/고객 여정지도.md` 각 단계 Pain Point, 특히 14절(단절 4명) |
| ~~JTBD 인터뷰 사전 단계~~ | ~~Job Statement 단위~~ | ~~"이 고객이 진보를 이루기 위해 수행하는 일(Job)은 무엇인가?"~~ | 07 JTBD 인터뷰가 아직 실행 전(`가람/0819/JTBD 분석 방법론.md`만 세팅됨)이라 이 행은 지금은 사용 불가 — 인터뷰 완료 후 이 문서에 갱신 |
...
| 단계 | 내용 | 이 프로젝트에서의 적용 |
| --- | --- | --- |
| 1. 후보 리스트업 | 점수화할 Pain/Job 후보를 모은다 | 2절에서 정한 소스(Core+Adjacent 8명의 Pain·Goal, 고객여정지도 Pain Point)에서 중복 제거해 후보 목록 작성 — 목표 5~10개(`방법론.md` 06 산출물 기준) |
...
| 3. 충족도(Satisfaction) 평가 | 기존 대안이 이 Pain을 얼마나 잘 해결해주는지 평가 | 뱅크샐러드·카드고릴라·엑셀 가계부·지인 추천 등 기존 대안(`JTBD 분석 방법론.md` 3-1절 대안 목록과 동일 소스)이 각 Pain을 얼마나 해소하는지 1~5점 척도 |
| 4. AOS 계산 | `Importance × (1 − Satisfaction/5)` 적용 | 계산 후 사분면 배치. 기준점(threshold)은 각 지표의 **중앙값(median)**을 사용(방법 A의 변형 — 고정 3.0 대신 실제 채점 표본의 중앙값), 표본이 20건 이상 쌓이면 방법 B(평균+0.5σ)로 전환 |
...
- [ ] Core+Adjacent 8명(이서연·김도윤·박서준·한지민·최유리·정하늘·오세훈·강민재)의 Pain·Goal에서 중복 제거해 5~10개 후보 확정
...
**연결 문서**: `가람/0819/페르소나 스펙트럼 분석 보고서.md` · `가람/0819/페르소나.md` · `가람/0819/고객 여정지도.md`(14절) · `가람/0819/Evidence Log.md` · `가람/0819/JTBD 분석 방법론.md` · `방법론.md` 06·5장(연결 규칙)
